...
# Parse the arguments
options=$(getopt -o h --long position:,size:,music_dir:,version,force-online,no-send-notifications,silent,help -- "$@")
eval set -- "$options"
...
get_album_offline() {
    # Get music location
    MUSIC_LOCATION=$(find $MUSIC_DIR -iname $(mpc current))

    # Performs a copy of the album found in the metadata of the song
    ffmpeg -i $MUSIC_LOCATION -an -vcodec copy $COVER
}
...
override-metadata() {
    ...
    ffmpeg -i $MUSIC_LOCATION -f ffmetadata data.txt
    
    artist=$(cat data.txt | grep 'artist')
...
find_album_art() {
    ...
    if [ "$ONLINE_ALBUM_ART" == true ];then
        ...
        [ ! "$SILENT" ] && echo "kunst: searching music metadata, biach gimme a sec-"
        get_cover_offline
    fi
...
main() {
    ...
    while true; do
        ...
        if [ ! "$NO_SEND_NOTIFICATIONS" ] ; then
            ...
            dunstify -h string:fgcolor:#2c2e3e '      ' -i $COVER "<span foreground='${color}' font_desc='UbuntuMono Nerd Font 22'><b>MPD Player</b></span>\n<span foreground='${color}' font_desc='Cartograph CF 24'><b>$YEET</b></span>"
        fi
